...
def salvar_resultado(dominio, ip):
    with open(NOME_ARQUIVO_SAIDA, 'a', encoding='utf-8') as f:
        f.write(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | {dominio} | {ip}\n")

def main():
    print("--- GERADOR DE SUBDOMÍNIOS DDNS (APENAS SALVA IPs ENCONTRADOS) ---")
    ...
    # 4. Resolver DNS e salvar resultados
    print("\n[3/4] Resolvendo DNS e salvando apenas os que existem...")
    encontrados = 0
    with ThreadPoolExecutor(max_workers=20) as executor:
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            if resultado:
                ...
                print(f"[ENCONTRADO] {dominio} -> {ip}")
                salvar_resultado(dominio, ip)
                encontrados += 1
            if (i+1) % 500 == 0:
                print(f"Progresso DNS: {i+1}/{len(todos_subdominios)} | Encontrados: {encontrados}")
    
    print(f"\n--- FINALIZADO ---")
    print(f"Total de domínios encontrados: {encontrados}")
    print(f"Todos os resultados foram salvos em '{NOME_ARQUIVO_SAIDA}' (domínio | IP).")
